...
- Route: `/login`
- State: light theme, default account/password values, account login tab selected, password masked, single sign-on tab hidden
...
Full-view comparison confirms the same two-column composition, 52.5% / 47.5% split, dark brand panel, light form panel, vertical positioning, form width, button hierarchy, and footer placement. The revised capture also reflects the screenshot feedback: larger typography, a single account-login tab, the account label, and horizontally aligned form labels/inputs.
...
- Copy and content: login title, helper text, account/password labels, security notice, support row, and footer metadata follow the screenshot while preserving the existing authentication flow; the single sign-on tab is intentionally hidden per the feedback screenshot.
...
- Revision: hid the single sign-on tab, changed “用户名” to “账号”, aligned labels horizontally with inputs, and increased page typography.
