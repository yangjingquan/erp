...
@router.post("/orders")
def create_order(payload: SalesOrderCreate, context: UserContext = Depends(get_current_user), db: Session = Depends(get_db)):
...
def create_delivery(order_id: str, context: UserContext = Depends(get_current_user), db: Session = Depends(get_db)):
...
@router.post("/returns")
def create_return(payload: SalesReturnCreate, context: UserContext = Depends(get_current_user), db: Session = Depends(get_db)):
...
@router.post("/quotes")
def add_quote(payload: SalesQuoteCreate, context: UserContext = Depends(get_current_user), db: Session = Depends(get_db)):
    return ok(serialize_quote(create_quote(db, payload, context)))


@router.post("/quotes/{quote_id}/{action}")
def quote_action(quote_id: str, action: str, context: UserContext = Depends(get_current_user), db: Session = Depends(get_db)):
...
def complete_delivery(delivery_id: str, context: UserContext = Depends(get_current_user), db: Session = Depends(get_db)):
