...
def create_lead(db, payload, context):
    row = CrmLead(org_id=context.org_id, owner_id=context.id, **payload); db.add(row); db.flush(); return row
...
def convert_lead(db, lead_id, context):
    ...
    opp = CrmOpportunity(org_id=context.org_id, name=row.name, customer_id=customer.id, owner_id=context.id); db.add(opp); db.flush()
...
def create_opportunity(db, payload, context):
    row = CrmOpportunity(org_id=context.org_id, owner_id=context.id, **payload); db.add(row); db.flush(); return row
