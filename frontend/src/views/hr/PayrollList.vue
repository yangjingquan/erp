...
async function load() { loading.value = true; try { const response = await listPayroll(validPeriod(period.value) ? period.value : undefined); rows.value = Array.isArray(response.data?.data) ? response.data.data : []; } catch { ElMessage.error("薪资列表加载失败"); } finally { loading.value = false; } }
