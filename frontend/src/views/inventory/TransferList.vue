...
function openCreate() { editingId.value = ""; form.from_warehouse_id = ""; form.to_warehouse_id = ""; form.items = [{ material_id: "", quantity: 1, unit_cost: 0 }]; dialogVisible.value = true; }
function openEdit(row: Row) { editingId.value = String(row.id); form.from_warehouse_id = row.from_warehouse_id; form.to_warehouse_id = row.to_warehouse_id; form.items = Array.isArray(row.items) && row.items.length ? row.items.map((it: Row) => ({ material_id: it.material_id, quantity: Number(it.quantity) || 0, unit_cost: Number(it.unit_cost) || 0 })) : [{ material_id: "", quantity: 1, unit_cost: 0 }]; dialogVisible.value = true; }
async function save() {
  ...
  saving.value = true;
  try { const response = editingId.value ? await updateInventoryTransfer(editingId.value, form) : await createInventoryTransfer(form); if (response.data.code !== 0) throw new Error(response.data.msg); ElMessage.success(editingId.value ? "调拨单已修改" : "调拨单已创建"); dialogVisible.value = false; await load(); }
  catch (error) { ElMessage.error(error instanceof Error ? error.message : "调拨单保存失败"); }
  finally { saving.value = false; }
}
async function removeTransfer(row: Row) { const id = String(row.id || ""); try { await ElMessageBox.confirm(`确认删除调拨单“${row.doc_no || id}”吗？删除后不可恢复。`, "删除确认", { type: "warning" }); actionLoading.value = id; const response = await deleteInventoryTransfer(id); if (response.data.code !== 0) throw new Error(response.data.msg); ElMessage.success("调拨单已删除"); await load(); } catch (error: any) { if (error !== "cancel" && error !== "close") ElMessage.error(error instanceof Error ? error.message : "删除失败"); } finally { actionLoading.value = null; } }
