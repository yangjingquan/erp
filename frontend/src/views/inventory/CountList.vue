...
async function save() { if (!form.warehouse_id || !form.items[0]?.material_id || form.items[0].actual_quantity < 0) { ElMessage.warning("请填写仓库、物料和有效实盘数量"); return; } saving.value = true; try { await createInventoryCount(form); ElMessage.success("盘点单已创建"); dialogVisible.value = false; await load(); } catch (error) { ElMessage.error("盘点单创建失败"); } finally { saving.value = false; } }
async function complete(row: Row) { const id = String(row.id || ""); if (!id) { ElMessage.error("盘点单缺少有效 ID，无法操作"); return; } try { await ElMessageBox.confirm(`确认完成盘点单“${row.doc_no || id}”吗？完成后会调整库存。`, "高危操作确认", { type: "warning" }); actionLoading.value = id; await completeInventoryCount(id); ElMessage.success("盘点单已完成"); await load(); } catch (error: any) { if (error !== "cancel" && error !== "close") ElMessage.error("盘点单完成失败"); } finally { actionLoading.value = null; } }
