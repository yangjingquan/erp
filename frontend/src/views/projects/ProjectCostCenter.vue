...
async function load() { loading.value = true; try { rows.value = unwrap(await listProjects()) || []; } catch (error) { ElMessage.error(error instanceof Error ? error.message : "项目加载失败"); } finally { loading.value = false; } }
