  ...
  <main class="login-page">
    ...
    <section class="login-panel">
      <div class="login-box">
        ...
        <div class="login-tabs" role="tablist" aria-label="登录方式"><button class="login-tab is-active" type="button" role="tab" aria-selected="true">账号登录</button><button class="login-tab" type="button" role="tab" aria-selected="false" disabled>单点登录</button></div>
        <el-form class="login-form" @submit.prevent="submit">
          <el-form-item label="用户名">
            <el-input v-model="username" placeholder="请输入用户名" size="large" autocomplete="username"><template #prefix><el-icon><User /></el-icon></template></el-input>
          </el-form-item>
          <el-form-item label="密码">
            <el-input v-model="password" type="password" show-password placeholder="请输入密码" size="large" autocomplete="current-password"><template #prefix><el-icon><Lock /></el-icon></template></el-input>
          </el-form-item>
...
.story-brand { display: flex; align-items: center; gap: 10px; color: #fff; font-size: 15px; }
...
.story-eyebrow, .welcome-eyebrow { color: #bc8874; font-size: 10px; font-weight: 700; letter-spacing: .16em; }
.story-copy h1 { margin: 15px 0 16px; color: #fffaf5; font-size: clamp(38px, 4vw, 58px); font-weight: 760; line-height: 1.13; letter-spacing: -.045em; }
.story-copy p { max-width: 520px; margin: 0; color: #b8aea7; font-size: 14px; line-height: 1.8; }
...
.feature-card strong { display: block; color: #f4eae2; font-size: 12px; }
.feature-card p { margin: 7px 0 0; color: #a99d94; font-size: 10px; line-height: 1.55; }
.story-footer { margin-top: auto; color: #8f837b; font-size: 10px; }
...
.login-box h2 { margin: 12px 0 7px; color: #3b342f; font-size: 28px; line-height: 1.2; letter-spacing: -.035em; }
.welcome-copy { margin: 0 0 25px; color: #a49a90; font-size: 12px; line-height: 1.6; }
.login-tabs { display: flex; gap: 21px; height: 37px; border-bottom: 1px solid #e7ddd2; }
.login-tab { position: relative; padding: 0 0 12px; border: 0; background: transparent; color: #a49a90; font-size: 12px; cursor: default; }
...
.login-form { margin-top: 18px; }.login-form :deep(.el-form-item) { margin-bottom: 15px; }.login-form :deep(.el-form-item__label) { height: auto; margin-bottom: 7px; color: #776b62; font-size: 11px; font-weight: 600; line-height: 1; }.login-form :deep(.el-input__wrapper) { min-height: 39px; border-radius: 9px; background: #fffdf9; box-shadow: 0 0 0 1px #e4d9ce inset; }.login-form :deep(.el-input__wrapper.is-focus) { box-shadow: 0 0 0 1px var(--erp-primary) inset, 0 0 0 3px rgba(198,109,75,.1); }.login-form :deep(.el-input__inner) { color: #4a4039; font-size: 12px; }.login-form :deep(.el-input__inner::placeholder) { color: #b9ada2; }.login-form :deep(.el-input__prefix) { color: #b1a59b; }.login-form :deep(.el-input__suffix) { color: #b1a59b; }
.form-helper { display: flex; align-items: center; justify-content: space-between; margin: -1px 0 17px; color: #a59a90; font-size: 10px; }.remember-note { display: inline-flex; align-items: center; gap: 6px; }.checkbox-mark { display: inline-block; width: 13px; height: 13px; border: 1px solid #ded3c8; border-radius: 4px; background: #fffdf9; }.forgot-link, .support-row button { padding: 0; border: 0; background: transparent; color: var(--erp-primary-dark); cursor: pointer; font-size: 10px; }
.submit { display: flex; align-items: center; justify-content: center; gap: 8px; width: 100%; min-height: 39px; border: 0; border-radius: 9px; background: var(--erp-primary); box-shadow: 0 8px 16px rgba(198,109,75,.2); font-size: 12px; font-weight: 700; }.submit:hover { background: #b95f42; }
.security-note { display: flex; align-items: flex-start; gap: 7px; margin-top: 17px; padding: 12px 11px; border-radius: 9px; background: #f8eed4; color: #a8874c; font-size: 10px; line-height: 1.5; }.security-note .el-icon { flex: 0 0 auto; margin-top: 1px; }
.support-row { display: flex; align-items: center; justify-content: space-between; margin-top: 21px; padding-top: 15px; border-top: 1px solid #eee5dc; }.support-row div { display: flex; flex-direction: column; gap: 4px; }.support-row strong { color: #756961; font-size: 10px; }.support-row span { color: #aaa097; font-size: 10px; }.support-row button { display: inline-flex; align-items: center; gap: 5px; }.login-meta { margin-top: 28px; color: #b8ada4; font-size: 9px; text-align: center; }
