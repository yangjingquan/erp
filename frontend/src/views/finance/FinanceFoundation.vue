...
import { accountTypeLabels, dimensionTypeLabels, statusLabel, tagTypeOf } from "../../utils/labels";
...
const dialog = ref("");
const { pagedRows: accountRows, page: accountPage, pageSize: accountPageSize, total: accountTotal, updatePageSize: updateAccountPageSize } = useClientPagination(accounts);
const { pagedRows: dimensionRows, page: dimensionPage, pageSize: dimensionPageSize, total: dimensionTotal, updatePageSize: updateDimensionPageSize } = useClientPagination(dimensions);
const { pagedRows: periodRows, page: periodPage, pageSize: periodPageSize, total: periodTotal, updatePageSize: updatePeriodPageSize } = useClientPagination(periods);
const { pagedRows: bankRows, page: bankPage, pageSize: bankPageSize, total: bankTotal, updatePageSize: updateBankPageSize } = useClientPagination(banks);
...
function editAccount(row: Row) { open("account"); Object.assign(form, row); }
async function removeAccount(row: Row) {
  try { await ElMessageBox.confirm(`确认删除会计科目“${row.code} ${row.name}”吗？系统会校验科目是否已被使用。`, "删除确认", { type: "warning" }); unwrap(await deleteFinanceAccount(String(row.id))); ElMessage.success("会计科目已删除"); await load(); }
  catch (error: any) { if (error !== "cancel" && error !== "close") ElMessage.error(error instanceof Error ? error.message : "会计科目删除失败"); }
}

async function save() {
  saving.value = true;
  try {
    if (dialog.value === "account") { const payload = { code: form.code, name: form.name, account_type: form.account_type, balance_direction: form.balance_direction, parent_id: form.parent_id, allow_posting: form.allow_posting }; unwrap(form.id ? await updateFinanceAccount(String(form.id), payload) : await createFinanceAccount(payload)); }
  ...
  <section class="page-stack">
    ...
    <el-card v-loading="loading" shadow="never">
      <el-tabs v-model="activeTab">
        <el-tab-pane label="会计科目" name="accounts"><div class="toolbar"><el-button type="primary" @click="open('account')">新增科目</el-button></div><el-table :data="accountRows" stripe><el-table-column prop="code" label="科目编码" /><el-table-column prop="name" label="科目名称" /><el-table-column label="科目类型"><template #default="scope">{{ accountTypeLabels[scope.row.account_type] || scope.row.account_type }}</template></el-table-column><el-table-column label="余额方向"><template #default="scope">{{ scope.row.balance_direction === 'debit' ? '借' : '贷' }}</template></el-table-column><el-table-column prop="allow_posting" label="允许记账"><template #default="scope">{{ scope.row.allow_posting ? '是' : '否' }}</template></el-table-column><el-table-column label="状态"><template #default="scope"><el-tag :type="tagTypeOf(scope.row.status)" effect="light">{{ statusLabel(scope.row.status) }}</el-tag></template></el-table-column><el-table-column label="操作" width="150"><template #default="scope"><el-button link type="primary" @click="editAccount(scope.row)">编辑</el-button><el-button link type="danger" @click="removeAccount(scope.row)">删除</el-button></template></el-table-column></el-table><ClientPagination v-model:page="accountPage" v-model:page-size="accountPageSize" :total="accountTotal" @update:page-size="updateAccountPageSize" /></el-tab-pane>
        <el-tab-pane label="核算维度" name="dimensions"><div class="toolbar"><el-button type="primary" @click="open('dimension')">新增维度</el-button></div><el-table :data="dimensionRows" stripe><el-table-column prop="code" label="维度编码" /><el-table-column prop="name" label="名称" /><el-table-column label="类型"><template #default="scope">{{ dimensionTypeLabels[scope.row.dimension_type] || scope.row.dimension_type }}</template></el-table-column><el-table-column label="必填"><template #default="scope">{{ scope.row.required ? '是' : '否' }}</template></el-table-column><el-table-column label="状态"><template #default="scope"><el-tag :type="tagTypeOf(scope.row.status)" effect="light">{{ statusLabel(scope.row.status) }}</el-tag></template></el-table-column></el-table><ClientPagination v-model:page="dimensionPage" v-model:page-size="dimensionPageSize" :total="dimensionTotal" @update:page-size="updateDimensionPageSize" /></el-tab-pane>
        <el-tab-pane label="会计期间" name="periods"><div class="toolbar"><el-button type="primary" @click="addPeriod">新增期间</el-button></div><el-table :data="periodRows" stripe><el-table-column prop="period" label="期间" /><el-table-column prop="start_date" label="开始日期" /><el-table-column prop="end_date" label="结束日期" /><el-table-column label="状态"><template #default="scope"><el-tag :type="tagTypeOf(scope.row.status === 'open' ? 'active' : scope.row.status)" effect="light">{{ statusLabel(scope.row.status === 'open' ? 'active' : scope.row.status) }}</el-tag></template></el-table-column><el-table-column label="操作"><template #default="scope"><el-button v-if="scope.row.status === 'open'" link type="warning" @click="changePeriod(scope.row, 'close')">结账</el-button><el-button v-else link type="primary" @click="changePeriod(scope.row, 'reopen')">重开</el-button></template></el-table-column></el-table><ClientPagination v-model:page="periodPage" v-model:page-size="periodPageSize" :total="periodTotal" @update:page-size="updatePeriodPageSize" /></el-tab-pane>
        <el-tab-pane label="银行账户" name="banks"><div class="toolbar"><el-button type="primary" @click="open('bank')">新增银行账户</el-button></div><el-table :data="bankRows" stripe><el-table-column prop="name" label="账户名称" /><el-table-column prop="bank_name" label="开户银行" /><el-table-column prop="account_no_masked" label="银行账号" /><el-table-column prop="currency" label="币种" /><el-table-column label="状态"><template #default="scope"><el-tag :type="tagTypeOf(scope.row.status)" effect="light">{{ statusLabel(scope.row.status) }}</el-tag></template></el-table-column></el-table><ClientPagination v-model:page="bankPage" v-model:page-size="bankPageSize" :total="bankTotal" @update:page-size="updateBankPageSize" /></el-tab-pane>
        <el-tab-pane label="固定资产" name="assets"><div class="toolbar"><el-button type="primary" @click="open('asset')">新增资产</el-button></div><el-table :data="assetRows" stripe><el-table-column prop="asset_code" label="资产编码" /><el-table-column prop="asset_name" label="资产名称" /><el-table-column prop="original_value" label="原值" /><el-table-column prop="accumulated_depreciation" label="累计折旧" /><el-table-column prop="net_value" label="净值" /><el-table-column prop="last_depreciation_period" label="最近计提期间" /><el-table-column label="操作"><template #default="scope"><el-button link type="primary" @click="depreciate(scope.row)">计提折旧</el-button></template></el-table-column></el-table><ClientPagination v-model:page="assetPage" v-model:page-size="assetPageSize" :total="assetTotal" @update:page-size="updateAssetPageSize" /></el-tab-pane>
      </el-tabs>
    </el-card>
