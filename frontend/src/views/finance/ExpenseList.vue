...
async function save() { if (!form.expense_type || form.amount <= 0) { ElMessage.warning("请填写费用类型和大于 0 的金额"); return; } saving.value = true; try { await createExpense(form); ElMessage.success("报销单已创建"); dialogVisible.value = false; await load(); } catch (error) { ElMessage.error("报销单创建失败"); } finally { saving.value = false; } }
async function createVoucher(row: Row) { const id = String(row.id || ""); if (!id) { ElMessage.error("报销单缺少有效 ID，无法生成凭证"); return; } try { await ElMessageBox.confirm(`确认根据报销单“${row.doc_no || id}”生成凭证吗？`, "操作确认", { type: "warning" }); await generateVoucher("expense", id); ElMessage.success("报销凭证已生成"); } catch (error: any) { if (error !== "cancel" && error !== "close") ElMessage.error("报销凭证生成失败"); } }
async function changeStatus(row: Row, action: "approve" | "settle") { const id = String(row.id || ""); if (!id) { ElMessage.error("报销单缺少有效 ID，无法操作"); return; } const label = action === "approve" ? "审核报销单" : "结算报销单"; try { await ElMessageBox.confirm(`确认${label}“${row.doc_no || id}”吗？`, "操作确认", { type: "warning" }); actionLoading.value = id; if (action === "approve") await approveExpense(id); else await settleExpense(id); ElMessage.success(`${label}成功`); await load(); } catch (error: any) { if (error !== "cancel" && error !== "close") ElMessage.error(`${label}失败`); } finally { actionLoading.value = null; } }
onMounted(load);
</script>
...
<template>
  <section>
    ...
    <el-space class="toolbar"><el-button type="primary" @click="openCreate">新建报销</el-button><el-button :loading="loading" @click="load">刷新</el-button></el-space>
    ...
    <el-table v-loading="loading" :data="rows" stripe><el-table-column prop="doc_no" label="报销单号" /><el-table-column prop="expense_type" label="费用类型" /><el-table-column prop="amount" label="金额" /><el-table-column prop="status" label="状态" /><el-table-column label="操作" min-width="260"><template #default="scope"><el-button v-if="scope.row.status === 'draft'" link type="primary" :loading="actionLoading === scope.row.id" @click="changeStatus(scope.row, 'approve')">审核</el-button><el-button v-if="scope.row.status === 'approved'" link type="warning" :loading="actionLoading === scope.row.id" @click="changeStatus(scope.row, 'settle')">结算</el-button><el-button v-if="scope.row.status === 'settled'" link type="primary" :loading="actionLoading === scope.row.id" @click="createVoucher(scope.row)">生成凭证</el-button></template></el-table-column></el-table>
