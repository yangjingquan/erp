...
<template>
  <section>
    ...
    <el-table v-loading="loading" :data="rows" stripe width="100%" fit><el-table-column prop="doc_no" label="报销单号" /><el-table-column prop="expense_type" label="费用类型" /><el-table-column prop="amount" label="金额" /><el-table-column prop="status" label="状态" /><el-table-column label="操作" width="260"><template #default="scope"><el-button v-if="scope.row.status === 'draft'" link type="primary" :loading="actionLoading === scope.row.id" @click="changeStatus(scope.row, 'approve')">审核</el-button><el-button v-if="scope.row.status === 'approved'" link type="warning" :loading="actionLoading === scope.row.id" @click="changeStatus(scope.row, 'settle')">结算</el-button><el-button v-if="scope.row.status === 'settled'" link type="primary" :loading="actionLoading === scope.row.id" @click="createVoucher(scope.row)">生成凭证</el-button></template></el-table-column></el-table>
