...
async function confirmAction(row: Row, action: "submit" | "approve" | "receipt") {
  ...
  try {
    ...
    if (action === "approve") await approvePurchaseOrder(id);
    if (action === "receipt") await createPurchaseReceipt(id);
...
<template>
  <section>
    ...
    <el-dialog v-model="dialogVisible" title="新建采购订单" width="560px">
